...
function get_vars(meta) if meta['include-auto'] then include_auto = true end end

--- Keep last heading level found
local last_heading_level = 0
function update_last_level(header) last_heading_level = header.level end
    ...
    local update_contents_filter = {
        ...
        CodeBlock = function(cb)
            if cb.attributes.include and path.is_relative(cb.attributes.include) then
                cb.attributes.include = path.normalize(path.join({
                    include_path, cb.attributes.include
                }))
            end
            return cb
        end
    ...
    for line in cb.text:gmatch('[^\n]+') do
        if line:sub(1, 2) ~= '//' then
            ...
            if not fh then
                ...
                contents = system.with_working_directory(path.directory(line),
                                                         function()
